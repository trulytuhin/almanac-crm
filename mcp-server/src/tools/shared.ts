...
// layer dumb). On a AlmanacApiError we return an `isError` result with
...
import { AlmanacApiError } from '../client.js';
...
/**
 * Wrap a tool handler so any AlmanacApiError becomes a clean, model-
...
export function handle<A>(
  ...
  return async (args: A) => {
    try {
      ...
      if (err instanceof AlmanacApiError) {
        return errorResult(`Almanac API error [${err.code}]: ${err.message}`);
      }
