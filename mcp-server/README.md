...
**[Almanac](https://github.com/trulytuhin/almanac-crm)** — the self-hostable
...
It's a thin wrapper over Almanac's public [`/api/v1`](../docs/public-api.md)
REST API. All auth, scoping, and rate limiting are enforced by your
Almanac instance — this server just exposes the API as MCP tools.

## Prerequisites

1. A running Almanac instance (your own self-hosted deploy).
...
| ------------------------- | -------- | -------------------------------------------------------------- |
| `ALMANAC_BASE_URL`          | yes      | Your instance URL, e.g. `https://crm.example.com`              |
| `ALMANAC_API_KEY`           | yes      | An API key from the dashboard                                  |
| `ALMANAC_ENABLE_WRITES`     | no       | `true` to expose contact writes + message sending             |
| `ALMANAC_ENABLE_BROADCASTS` | no       | `true` to expose mass broadcasts (needs `ALMANAC_ENABLE_WRITES`) |
...
```jsonc
{
  "mcpServers": {
    "almanac": {
      "command": "npx",
      "args": ["-y", "almanac-crm-mcp"],
      "env": {
        "ALMANAC_BASE_URL": "https://crm.example.com",
        "ALMANAC_API_KEY": "almanac_live_xxxxxxxxxxxxxxxxxxxxxxxx"
      }
    }
  }
}
...
"env": {
  "ALMANAC_BASE_URL": "https://crm.example.com",
  "ALMANAC_API_KEY": "almanac_live_xxxxxxxxxxxxxxxxxxxxxxxx",
  "ALMANAC_ENABLE_WRITES": "true",
  "ALMANAC_ENABLE_BROADCASTS": "true"
}
```
...
1. **Read-only by default.** Write and broadcast tools are not even
   ...
   `ALMANAC_ENABLE_WRITES` / `ALMANAC_ENABLE_BROADCASTS`.
2. **API-key scopes.** Whatever the guards allow, your Almanac instance
