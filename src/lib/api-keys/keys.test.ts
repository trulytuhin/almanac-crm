...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('almanac_live_abc')).toBe(hashApiKey('almanac_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('almanac_live_abc')).not.toBe(hashApiKey('almanac_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('almanac_live_xyz');
