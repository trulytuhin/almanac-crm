...
//
// Every delivery carries an `X-Almanac-Signature` header so receivers
// can verify the request really came from Almanac and wasn't tampered
// with or replayed. The scheme is Stripe-style:
//
//   X-Almanac-Signature: t=<unix_seconds>,v1=<hex HMAC-SHA256>
//
...
/**
 * Build the `X-Almanac-Signature` header value for `rawBody`, signed
...
/**
 * Verify a signature header. Exposed so an Almanac-to-Almanac integration
