 ...
 *  Almanac is India-first, so new accounts start in rupees. */
...
/** Rupee amounts read best with Indian grouping; others follow the
 *  viewer's locale. */
 ...
 * Rupees count in lakh and crore: "₹2.5L", "₹1.2Cr".
 */
export function formatCurrencyShort(
  ...
  if (code === "INR") return `${symbol}${formatIndianCompact(value)}`;
  return `${symbol}${formatCompactNumber(value)}`;
}

/** 1_20_00_000 → "1.2Cr", 2_50_000 → "2.5L", 3_400 → "3.4k". */
