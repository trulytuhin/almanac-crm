...
Almanac ships a [Model Context Protocol](https://modelcontextprotocol.io)
...
[`almanac-crm-mcp`](https://www.npmjs.com/package/almanac-crm-mcp). Under the hood
...
data or send messages, add `"ALMANAC_ENABLE_WRITES": "true"` (and
`"ALMANAC_ENABLE_BROADCASTS": "true"` for mass sends) to `env`.
