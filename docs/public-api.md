...
The public API lets you drive your Almanac instance from your own
...
```
Authorization: Bearer almanac_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** Almanac
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer almanac_live_xxx"
```
...
Rather than polling, register an endpoint and Almanac will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; Almanac keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer almanac_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/almanac", "events": ["message.received"] }'
...
Headers: `X-Almanac-Event`, `X-Almanac-Webhook-Id`, and `X-Almanac-Signature`.

### Verifying the signature

`X-Almanac-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages Almanac stores (inbox + API sends), not broadcast-only
...
the original roadmap issue. Future ideas
