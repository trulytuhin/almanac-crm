# Several WhatsApp Business Accounts on one Almanac deployment
...
| --- | --- | --- |
| Phone Number ID, WABA ID, access token, verify token, two-step PIN | `whatsapp_config` (one row per Almanac account, token encrypted) | per account |
...
is subscribed to. Almanac checks that signature against
...
3. Each Almanac account opens Settings → WhatsApp connection and enters
...
On save, Almanac verifies the number with the token, registers it
...
and Almanac fans them out by `phone_number_id`.

Constraints:

* One phone number can be connected to one Almanac account only — the
...
* The **verify token** typed into the Meta App's webhook settings must
  equal the verify token saved by at least one Almanac account; the
...
3. Each Almanac account connects its number exactly as in setup A, using a
...
* Create image-header templates directly in WhatsApp Manager — once
  approved, Almanac lists and sends them like any other template.
