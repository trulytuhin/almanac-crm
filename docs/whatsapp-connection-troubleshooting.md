...
fails, Almanac now tells you *which* call failed, *which field* to check,
...
## Common causes and what Almanac shows
...
> Enter the two-step verification PIN below and save again so Almanac can
...
Nothing in Almanac fixes this — it is a Meta policy or verification
state.

> Meta has restricted or locked this WhatsApp Business Account, so
> nothing in Almanac can connect it. Open Meta Business Manager → Account
...
4. Confirm the webhook itself: in the Meta App → WhatsApp →
   ...
   rejected with a 401 before Almanac looks at it.
